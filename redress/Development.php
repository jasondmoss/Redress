...
if ($devel) {
    ...
    foreach ([ 'admin_head', 'wp_head' ] as $action) {
        add_action($action, 'printDevServerCommentMessage');
    }

    foreach ([ 'admin_footer', 'login_footer', 'wp_footer' ] as $action) {
        add_action($action, 'displayDevelopmentBar');
    }
}
 ...
 * @access public
 */
function printDevServerCommentMessage()
{
    echo <<<MSG
 ...
 * @access public
 */
function displayDevelopmentBar()
{
 ...
 */
function dump($var, $label = '', $exit = false)
{
